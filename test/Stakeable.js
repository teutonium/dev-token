...
contract("TeutToken", async accounts => {
    ...
    it("calculate rewards", async() => {
        ...
        await teutToken.stake(1000, {from: owner});
        await helper.advanceTimeAndBlock(3600*20);

        summary = await teutToken.hasStake(owner);
    ...
    it("reward stakes", async() => {
        ...
        await teutToken.mint(accounts[3],1000);
        let initial_balance = await teutToken.balanceOf(staker);
        // Make a stake on 200, fast forward 20 hours, claim reward, amount should be Initial balanace +4
        await teutToken.stake(200, {from: staker});
        await helper.advanceTimeAndBlock(3600*20);

        let stakeSummary = await teutToken.hasStake(staker);
        let stake = stakeSummary.stakes[0];
        // Withdraw 100 from stake at index 0 
        await teutToken.withdrawStake(100, 0, { from: staker});

        // Balance of account holder should be updated by 104 tokens
        let after_balance = await teutToken.balanceOf(staker);
        ...
        let second_balance = await teutToken.balanceOf(staker);
